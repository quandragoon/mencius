...
func (kv *ShardKV) Catchup(seqNum int, opType Type, reconfig bool) {
  ...
  for curSeq < seqNum {
    ...
    if decided {
      ...
      if ok {
        if val.OpType == PUT {
          dbVal, dbOk := kv.keyvalues[kv.config.Num][val.Key]
          newval := ""
          previousVal := ""
          if val.DoHash {
            curVal, ok := kv.keyvalues[kv.config.Num][val.Key]
            if !ok {
          ...
          if !dbOk {
            //if (dbVal.ConfigNum < val.(Op).Num && val.(Op).Num <= kv.config.Num) {
              ...
              kv.keyvalues[kv.config.Num][val.Key] = RequestValue{newval, val.ClientID, val.RequestTime, curSeq, kv.config.Num}
            ...
            if dbVal.RequestTime.Before(val.RequestTime) { // At most once guarantee
              ...
              kv.keyvalues[kv.config.Num][val.Key] = RequestValue{newval, val.ClientID, val.RequestTime, curSeq, val.Num}
              kv.putRequests[val.ClientID] = DuplicatePut{val.RequestTime, previousVal}
            } else {
